...
#hierarchial
distmat <- dist(pmatrix, method="euclidean") #create distance matrix
pfit <- hclust(distmat,method="ward.D") #cluster
plot(pfit, labels=protein$Country) #dendrogram
...
nComp <- 2
# rotate the data into the space described by the principal 
#components. We only want the projection on the first two axes.
...
#Compute the Calinski and Harabasz score. It is also known as the Variance Ratio 
#Criterion. The score is defined as ratio between the within-cluster dispersion 
...
#"unscale" data points (put them back in the coordinates of the original ds). 
unscale <- function(scaledpt, centervec, scalevec) {    
  scaledpt*scalevec + centervec
}

#Unscale the first centroid. It corresponds to original distribution 2. 
